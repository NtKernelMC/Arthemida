/*
	...
	+ Проверки адресов возврата с важных игровых функций
...
extern "C" void __stdcall HandleApc(PVOID ApcRoutine, PVOID Argument, PCONTEXT Context)
{
	...
	{
		if (SearchForSingleMapMatch<PVOID, const char*>(flt.ApcList, Routine)) return true;
		return false;
	};
	if (IsRoutineForbidden(Argument))
	{
		...
		strcpy(ForbiddenName, SearchForSingleMapMatchAndRet(flt.ApcList, Argument).c_str());
